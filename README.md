...
| **Web console** | served by the relay at `/console` | Live view of channels, documents, and requests. The Navigator tab browses the disk index three levels deep and drives the live preview: drag to pan, scroll to zoom, or use the arrow/zoom/fit buttons (they move Figma's real viewport). |
| **Live preview stream** | `src/webrtc/` + `src/window_agent/` | The preview is WebRTC: a per-window agent (ScreenCaptureKit + hardware H.264, ~0.2-0.8 Mbps) feeds RTP to each viewer, browser ⇄ capture machine directly (STUN only, no TURN — on ICE failure the console falls back to the old 2s JPEG frames). Quality presets (fps/resolution/bitrate) apply live from the console and persist per browser. **Control mode** (opt-in toggle) sends clicks and an allowlisted key set (`\` UI toggle, Esc, arrows, zoom shortcuts) into the real window — delivered via `CGEventPostToPid` with brief app activation and focus restore, so the operator's cursor never moves and their focus comes back within a second. Input needs an Accessibility grant for the relay process on the capture machine. |
